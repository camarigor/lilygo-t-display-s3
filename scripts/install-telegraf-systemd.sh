...
echo "→ alvo: TELEGRAF_HOSTNAME=${HOST_ID} MQTT=${MQTT_HOST}:${MQTT_PORT} telegraf=${TELEGRAF_VERSION_PIN}.*"
...
TMP_ENV="$(mktemp)"
{
  ...
  echo "# Preencha MQTT_PASS_COLLECTOR com o valor de secrets/collector-<TELEGRAF_HOSTNAME>.pass"
  echo "# do workstation que detém os secrets, e reinicie o service."
  echo "TELEGRAF_HOSTNAME=${HOST_ID}"
